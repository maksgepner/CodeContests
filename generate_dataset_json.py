...
def _generate_json(filenames):
  ...
  for problem in _all_problems(filenames):
    data = {'problem_name': problem.name, 'rating': problem.cf_rating, 'tags': list(problem.cf_tags), 'problem_description': problem.description, 'solutions': []}
    for solution in problem.solutions:
      data['solutions'].append({'code': solution.solution, 'language': contest_problem_pb2.ContestProblem.Solution.Language.Name(solution.language).lower(), 'is_correct': True})
    for solution in problem.incorrect_solutions:
      data['solutions'].append({'code': solution.solution, 'language': contest_problem_pb2.ContestProblem.Solution.Language.Name(solution.language).lower(), 'is_correct': False})
